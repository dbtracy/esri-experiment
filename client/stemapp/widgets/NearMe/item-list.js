...
define([
  ...
  return declare([_WidgetBase, Evented], {
    ...
    _createFilterButton: function (parentNode) {
      var filterOptions, filterParent, isFilterButton = true;
      ...
      this.filterButton = domConstruct.create("div", {
        "class": "esriCTFilterIconContainer",
        "title": this.nls.filterTitle,
        "aria-label": this.nls.filterTitle,
        "tabindex": "0",
        "role": "button"
      }, filterParent);
      //Set the last focus node class for filter button if exist
      if (isFilterButton) {
        jimuUtils.initLastFocusNode(this.domNode, this.filterButton);
      }
      ...
      this.own(on(this.filterButton, "click", lang.hitch(this, function () {
        this.emit("show-filter");
      })));
      this.own(on(this.filterButton, 'keydown', lang.hitch(this, function (evt) {
    ...
    _filterConfiguredLayer: function (featureDeferArr) {
      ...
      for (var i = 0; i < this._operationalLayers.length; i++) {
        ...
        this._createItemTemplate(this._operationalLayers[i], featureDeferArr,
          this._operationalLayers.length - 1 === i);
      }
    ...
    * @memberOf widgets/NearMe/item-list
    **/
    _createItemTemplate: function (operationalLayer, featureDeferArr, isLastFeature) {
      var templateDiv;
      templateDiv = domConstruct.toDom(this._itemListTemplate).childNodes[0];
      ...
      if (isLastFeature) {
        domClass.add(templateDiv, "esriCTLastLayerFocusNode");
      }
    ...
    **/
    _onFeatureCountComplete: function () {
      ...
      if (this._isNoFeature) {
        ...
        var noFeatureFoundMessage = this.config.noFeatureFoundMessage;
        noFeatureDiv = domConstruct.create("div", {
          ...
          "innerHTML": noFeatureFoundMessage ? noFeatureFoundMessage : this.nls.noFeatureFoundText
    ...
    */
    _getPointSymbol: function (graphic, layer) {
      ...
      if (this.config.highlightColor) {
        ...
        highlightColor = new Color([0, 255, 255, 1]);
      }
