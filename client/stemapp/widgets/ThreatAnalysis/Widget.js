...
define([
  ...
  function (
    ...
    return declare([BaseWidget, dijitWidgetBase, dijitWidgetsInTemplate], {
      ...
      resize: function () {
        var nl = query(".dijitSelectLabel", this.threatType.domNode);
        if (nl) {
          array.forEach(nl, function (node) {
            var parentNodeWidth = this.unitType.domNode.clientWidth;
            var minParentWidth = 151;
            var diff = parentNodeWidth - minParentWidth;
            var minWidth = 112;
            var finalWidth = (minWidth + diff) > minParentWidth ? (minWidth + diff) : minWidth;
