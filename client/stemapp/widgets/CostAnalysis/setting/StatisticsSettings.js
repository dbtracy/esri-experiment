  ...
  return declare([BaseWidget, _WidgetsInTemplateMixin], {
    ...
    _updateStatisticsTable: function (updatedRowDetails) {
      ...
      if (updatedRowDetails.layerId) {
        ...
        // before adding layer in dropdown, check if its editable.
        // only editable layer needs to be added
        if (this.layerInfosObj) {
          layerInfo = this.layerInfosObj.getLayerInfoById(layerIdToBeAdded);
          if (layerInfo && layerInfo.layerObject) {
            var isLayerEditable = this._checkEditCapabilities(layerInfo.layerObject);
            if (!isLayerEditable) {
              layerIdToBeAdded = null;
            }
          }
        }
